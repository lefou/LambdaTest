...
By default `expectXXX`-methdos fail fast, which means the first failing assertion will end the whole test.
...
* `intercept` - Assert that a code block throws an Exception of the given type and optional with an message matching a given regular expression. Returns the throw exeption for further analysis
