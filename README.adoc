...
image:https://api.codacy.com/project/badge/Grade/e886bd7ca9784ecfb00fe8afb59b8909["Codacy code quality", link="https://www.codacy.com/app/lefou/LambdaTest?utm_source=github.com&utm_medium=referral&utm_content=lefou/LambdaTest&utm_campaign=Badge_Grade"]
...
* Useful assertion message and difference highlighting in `assertXXX` and `expectXXX`-methods
...
- divide by zero
- demo of a fail FAILED
java.lang.AssertionError: Strings differ at index 3. Actual is too long. Expected "yes" but was "yes and no".
...
...
== Writing assertions with `Assert` and `Expect`

{lambdatest} provides a set of assertions to check for your expectations.
...
* Nice output of differences between expected and actual values. Especially for string and varous collection types
...
* `assertXXX`-methods via `de.tobiasroeser.lamdatest.Assert` to check and compare actual values
* `expectXXX`-mehtods via de.tobiasroeser.lambdatest.Expect` - same as `assertXXX`, but can be configured to more advanced use, e.g. non-fail-fast error reporting
* `intercept`-methods via `de.tobiasroeser.lambdatest.Intercept` to intercept expected exceptions
...
* Released JAR files are now proper OSGi bundles
