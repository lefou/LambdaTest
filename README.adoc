...
* Changed handling of the "optional" msg-paramter in `Assert.assertXXX` and `Expect.expectXXX` methods.
  If given, the msg-parameter does no longer replace the generic assertion message, instead both messages are shown, first the given message, then the generic message.
* Better handle arrays with primitive types. Is some situations, a failing assert could throw a `ClassCastExcpetion`.
